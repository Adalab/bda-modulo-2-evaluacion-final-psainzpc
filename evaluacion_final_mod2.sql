...
SELECT title AS título, DESCRIPTION
	FROM film_text
    WHERE description LIKE '%dog%' OR description REGEXP '\\bcat\\b'; -- \\b marca inicio y din de palabra y la busca como palabra completa.
...
USING (actor_id)
WHERE film_id IS NULL; -- si hubiera algún actor que no estuviera asociado a pelicula, film_is apareceria como NULL
...
SELECT  CONCAT(a1.first_name, " ", a1.last_name) AS "ACTOR/ACTRIZ 1",
	...
	WITH peliculas_compartidas AS (SELECT actor_id, film_id 
